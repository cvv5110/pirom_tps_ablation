...
\begin{align}
    ...
    &= \begin{array}[t]{@{}l@{\;}c@{\;}l@{}}
       ...
       & + &
       \underbrace{\vPhiplus\vA(\vu)^{-1}\vC(\vu)\vu - \bvA(\vw)^{-1}\bvC(\bvu)\bvu}_{\text{\#2}} \\
       & + &
       \underbrace{\vPhiplus\vA(\vu)^{-1}\vf(t) - \bvA(\vw)^{-1}\bvf(t)}_{\text{\#3}}
       \end{array}
\end{align}
\end{subequations}
The magnitude analysis for terms \#1 and \#3 can be found in Ref.~\cite{VargasVenegas2025}. The analysis for term \#2 is presented next. Let $\vD(\bvu) = \vA(\vPhi\bvu)^{-1}\vP\vC(t,\vPhi\bvu)$, then,
\begin{subequations}
    \begin{align}

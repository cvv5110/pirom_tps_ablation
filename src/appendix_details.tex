...
\begin{equation}
    \vA_i\dot{\vu}_i = \left(\vB_i + \vC_i\right)\vu_i + \sumneighbordirichlet\left(\vB^i_{ij}\vu_i + \vB^{j}_{ij}\vu_j\right) + \vf_i(t),\quad\text{for }i=1,2,\dots,M\label{eqn_element_wise_model}
\end{equation}
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
Detailed derivations for the \#1, \#2, and \#4 terms can be found in Ref.~\cite{VargasVenegas2025} for the case of temperature-varying matrices. The effects of coarse-graining on the advection term \#3 are analyzed next.
...
\end{align}
\end{subequations}
The magnitude analysis for terms \#1 and \#3 can be found in Ref.~\cite{VargasVenegas2025}. The analysis for term \#2 is presented next. Let $\vD(\bvu) = \vA^{-1}\vP\vC(\vPhi\bvu)$ so that,
\begin{subequations}
    \begin{align}
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
where term \#2 is due to the approximation of non-uniform temperature as constants, and term \#1 is the error in the advection dynamics due to coarse-graining. 

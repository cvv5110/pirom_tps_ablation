...
In this section, the proposed PIROM approach is applied to the analysis of thermo-ablative multi-layered hypersonic TPS. The performance of the PIROM is evaluated in terms of the three corners of the ITM in Fig.~\hl{x}, across a wide range of boundary condition and SRM model parametrizations. The results show PIROM to be a promising candidate for the solution of the impossible trinity of modeling.
...
\begin{equation}
    q(x,t;\xibc) = \xi_0e^{\xi_1 x}e^{\xi_2 t},\:\forall x\in\Gamma_{i,q},\quad \dot{w}_i(z_{u,i};\xisrm) = \alpha_i\left(z_{u,i} - u_{0,i}\right)
\end{equation}

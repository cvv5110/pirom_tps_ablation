...
The next step involves coupling the physics-infused model in \cref{eqn_extended_markovian_system} with the SVM in \cref{eqn_svm} to define the PIROM for ablating TPS. To this end, define the observables as the surface temperature $\vz_u\in\mathbb{R}^{\tN}$ and displacements $\vz_w\in\mathbb{R}^{\tN}$ for $\tN\leq N$ ablating components to define the observable vector as $\vz=\left[\vz_u,\vz_w\right]^\top\in\mathbb{R}^{n_z}$ with $n_z=2\tN$ as the total number of observables.

...
At some time $t$, the mesh displacement field $\vd_m\in\mathbb{R}^d$ is described by the steady-state pseudo-elasticity equation, which models the mesh as a fictitious elastic solid that deforms according to the prescribed boundary displacements. The mesh velocities are obtained by differentiating the mesh displacements with respect to time, i.e., $\vv_m(x,t) = \dot{\vd}_m$, which appear in the advection term of the energy equation in \cref{eqn_thermal_pde} and is approximated using finite differences in the coupled simulation. The governing equation for mesh displacements is,
\begin{subequations}
    \begin{align}
...
\paragraph*{Pseudo-Elasticity Equation} The \textit{pseudo-elasticity equation} is spatially discretized using the standard Galerkin FEM method on a structured mesh with quadrilateral elements. Define the scalar basis functions $\left\{\psi_q(x)\right\}_{q=1}^{Q}$ nodal variables $\left\{\vd_q\right\}_{q=1}^{Q}$ for the mesh displacements, where $Q$ is the number of basis functions. Express the mesh displacements $\vd$ at time $t$ as,
\begin{equation}
    \vd(x,t) \approx \sum_{q=1}^{Q} \psi_q(x) \vd_q\label{eqn_mesh_fem_approximation}
\end{equation}
Substituting into the variational form the \textit{steady} pseudo-elasticity equation, the following linear system of equations is obtained for the nodal displacements at each time step,
\begin{equation}
    \vK\vd = \vg\label{eqn_fem_elasticity}
\end{equation}
where $\vd$ is the global displacement vector, $\vK\in\mathbb{R}^{dQ\times dQ}$ the global stiffness matrix of dimension $d$, defined by the volume integrals over the domain $\Omega$ provided the elasticity tensor $\mathbb{D}$, and $\vg$ is the global force vector due to the Dirichlet boundary conditions on the heated $\Gamma_q$ and unheated $\Gamma_T$ boundaries.

\paragraph*{Coupled Full-Order Model} The temperature-dependent mesh motion on the heated boundary $\Gamma_q$ naturally defines a two-way coupling scheme between the energy and pseudo-elasticity FEM models. At each time step, the mesh displacements $\vd_m$ are obtained by solving \cref{eqn_fem_elasticity}, where the Dirichlet boundary conditions on $\Gamma_q$ are computed by integrating the surface recession velocity provided by the surface temperature from the energy equation. Then, the mesh velocity $\dot{\vd}_m=\vv_m$ is computed using finite differences, which is used as an input into the advection matrix $\vC(\vu)$ in \cref{eqn_full_dg}. Finally, the temperature field is advanced in time for the next time step using \cref{eqn_full_dg} based on an implicit time-integration scheme. This procedure is repeated until the final time $t_f$ is reached.

...
This section presents the problem of thermo-ablative modeling for a non-decomposing ablating TPS, subjected to extreme hypersonic heating. Two different but mathematically connected solution strategies are provided: (1) a high-fidelity full-order model (FOM) based on a finite element method (FEM), and (2) a low-fidelity reduced-physics model (RPM) based on a lumped capacitance model (LCM) and a one-dimensional surface velocity model (SRM). The FOM is computationally expensive but provides the highest fidelity, while the RPM is computationally efficeint but has low predictive fidelity; both models are amenable to high-dimensional design variables. The RPM is used in the subsequent sections for deriving the PIROM.
